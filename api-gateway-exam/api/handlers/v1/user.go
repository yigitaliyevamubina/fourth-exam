...
import (
	...
	pb "exam/api-gateway/genproto/user_service"
...
func (h *handlerV1) Register(c *gin.Context) {
	...
	exists, err := h.serviceManager.UserService().CheckField(ctx, &pb.CheckFieldReq{
		Field: "email",
		Value: body.Email,
	})
...
func (h *handlerV1) Verify(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, models.ResponseError{
			...
			Message: "code is expired or not registered before, try again.",
		})
		h.log.Error("Code is expired or not registered before, TTL is over.")
		return
	}
...
func (h *handlerV1) Login(c *gin.Context) {
	...
	respUser, err := h.serviceManager.UserService().Get(ctx, &pb.GetRequest{Email: body.Email})
	if err != nil {
		c.JSON(http.StatusBadRequest, models.ResponseError{
			Code:    ErrorCodeInternalServerError,
			Message: err.Error(),
	...
	if !etc.CompareHashPassword(respUser.Password, body.Password) {
		c.JSON(http.StatusBadRequest, models.ResponseError{
...
func (h *handlerV1) CreateUser(c *gin.Context) {
	...
	if err != nil {
		...
		h.log.Error("cannot marshal user json", logger.Error(err))
		return
	}
...
func (h *handlerV1) GetUserById(c *gin.Context) {
	...
	if id == "" {
		if username != "" {
			...
			c.JSON(http.StatusBadRequest, models.ResponseError{
				Code:    ErrorBadRequest,
				Message: "provide either id or username",
			})
			return
		}
	} else {
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*time.Duration(h.cfg.CtxTimeOut))
	defer cancel()
	response, err := h.serviceManager.UserService().Get(ctx, &req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, models.ResponseError{
			Code:    ErrorCodeInternalServerError,
			Message: err.Error(),
...
// @Param UserInfo body models.UserRequest true "Update User"
// @Success 201 {object} models.UserModel
// @Failure 400 string Error models.ResponseError
// @Failure 500 string Error models.ResponseError
...
func (h *handlerV1) DeleteUser(c *gin.Context) {
	...
	_, err := h.serviceManager.UserService().Delete(ctx, &req)
...
// @Param orderBy query string false "orderBy" Enums(first_name, last_name, username, email, bio, website, created_at, updated_at) "Order by"
// @Param is_active query bool false "is_active" Enums(true, false)
// @Success 201 {object} models.UserWithProducts
// @Failure 400 string Error models.ResponseError
// @Failure 500 string Error models.ResponseError
func (h *handlerV1) GetAllUsers(c *gin.Context) {
	...
	response, err := h.serviceManager.UserService().List(ctx, &req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": err.Error(),
		})
...
// @Accept json
// @Produce json
// @Param refresh-token body models.RefreshTokenUpdateReq true "refresh-token"
// @Success 201 {object} models.UserModel
// @Failure 400 string Error models.ResponseError
// @Failure 500 string Error models.ResponseError
func (h *handlerV1) UpdateRefreshToken(c *gin.Context) {
	var (
		jspbMarshal protojson.MarshalOptions
		body        models.RefreshTokenUpdateReq
	)
	jspbMarshal.UseProtoNames = true
	...
	respUser, err := h.serviceManager.UserService().UpdateRefresh(ctx, &pb.UpdateRefreshReq{
		UserId:       cast.ToString(claims["sub"]),
		RefreshToken: refresh,
	})
	if err != nil {
